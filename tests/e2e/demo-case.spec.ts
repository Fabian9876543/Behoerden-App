 ...
 */
test("Demo-Vorgang anlegen und oeffnen", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Weiterbewilligung Buergergeld/ })).toBeVisible();
  ...
  // Drei Aufgaben, zwei benoetigte Unterlagen, ein Formular.
  ...
  await expect(page.getByText("Kontoauszuege der letzten drei Monate")).toHaveCount(2);
  await expect(page.getByText("Weiterbewilligungsantrag Buergergeld")).toBeVisible();
});
